				   ...
				   struct buffer_head *bh_result, int create)
{
	 ...
	 */
	if (index->extents[iblock].count == 0) { /* count == 0 means unallocated */
		if (!create) {
			ret = 0;
			goto brelse_index;
		}
	...
	if (ret < len) {
		...
		if (nr_blocks_old > inode->i_blocks) {
			...
			for (i = inode->i_blocks - 1; i < nr_blocks_old - 1;
				...
				index->extents[i].start = 0; /* mark as free */
									 ...
									 uint32_t *phys_out)
{
	...
	if (hole_count == 1) {
		...
		// Écriture à la FIN du trou {0, N} → {0, N-1}, {réel, 1}
	...
	while (len > 0) {
		...
		if (phys_block == OUICHEFS_HOLE_BLOCK) {
			...
			if (ci->i_reserved_count > 0) {
				...
				new_bno = ci->i_reserved_start; /* next reserved block */
				ci->i_reserved_start++;
				ci->i_reserved_count--;
			} else {
				/* Plus de reserve */
			...
			if (ei > 0) {
				struct ouichefs_extent *last = &index->extents[ei - 1];
